...
function update() {
    ...
    if (modoAuto == false && bala.position.x > 0) {
        let estatus = (estatusAire === 1 && estatuSuelo === 0) ? 1 : 0;
        saveDataToCSV(despBala, velocidadBala, estatus);
...
function saveDataToCSV(despBala, velocidadBala, estatus) {
    // Acumular los datos en el array
    datosCSV.push(`${despBala},${velocidadBala},${estatus}`);
}

function exportarCSV() {
    // Agregar los encabezados
    let csvContent = `Desplazamiento Bala,Velocidad Bala,Estatus\n`;
    
    // Agregar las filas de datos
    csvContent += datosCSV.join("\n");
